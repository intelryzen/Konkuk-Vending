...
class AdminFunctionParser(BaseParser):
    def parser(self, input: str ) -> tuple[bool,any]:  
        ...
        #슬롯의 정보 가져오기
        '''
        for i in range(len(c.slot_list)):
                s_num.append(c.slot_list[i].number) 
                s_drink_num.append(c.slot_list[i].drinknum)  
                s_drink_count.append(c.slot_list[i].count)
        '''
        ...
        # 5번 메뉴
            ...
            # 5번 메뉴 인자 개수 확인
            if len(parts) != 5:
                return False, "오류: 4개의 인자가 필요합니다. 첫번째 인자는 번호, 두번째 인자는 이름, 세번째 인자는 가격, 네번째 인자는 개수를 입력해주세요.(음료수 추가)"
