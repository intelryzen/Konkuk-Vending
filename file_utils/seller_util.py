...
def Load_Admin(sellerFilePath, Admin, Admin_List): #관리자 파일 로드
	"""
	아이디 비밀번호 이후 다른 문자에 대한 예외 처리 혹은 에러 처리가 필요함
	"""
	with open(sellerFilePath, 'r') as file:
		for line in file:
...
def Save_Admin(sellerFilePath, Admin, Admin_List): #관리자 파일 생성/저장
	"""
	저장하는 과정에서 저장에 대한 에러는 무시하는것으로 하는것이 맞는지 확인 필요함
	"""
	with open(sellerFilePath, 'w') as file:
